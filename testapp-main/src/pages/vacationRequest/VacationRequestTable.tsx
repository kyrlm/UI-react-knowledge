...
const VacationRequestTable = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:8081/api/getAllVacationRequests`)
      ...
      renderCell: (cellValues) => {
        return (
          <>
            <IconButton
              color="primary"
              onClick={() => navigate(`/vacationRequest/${cellValues?.row?.id}`)}
            >
              <ReadMoreIcon />
            </IconButton>
            <IconButton
              ...
              onClick={() => {
                ...
                fetch(`http://localhost:8081/api/deleteVacationRequestById/${cellValues?.row?.id}`, {
                  method: "DELETE",
                })
                 //.then((res) => res.json())
                  .then(() => {
  ...
  return (
    <div>
      ...
      >
        ...
        <IconButton color="primary" onClick={() => navigate(`/vacationRequest/new`)}>
          <AddIcon />
        </IconButton>
      </div>
